 ...
 * <p>
 * Key Amiga timing facts reproduced here: - PAL  clock: 3_546_895 Hz  (most MODs are mastered on PAL) - NTSC clock:
 * 3_579_545 Hz - Default BPM: 125, default Speed (ticks/row): 6 - CIA timer tick interval (µs) = 2_500_000 / BPM
 * (Protracker formula) - Rows per pattern: 64, channels: 4
 * <p>
 * Output: any sample rate you choose (e.g. 44100, 48000, 96000 Hz). Mixing is done in software at that rate;
 * period→frequency conversion uses the real Amiga clock so pitch is always correct regardless of output rate.
 */
public class ProTrackerPlayer05 {
     ...
     * <p>
     * ProTracker does NOT apply finetune with floating-point math — it looks up the pre-tuned period from this table,
     * exactly as the original source does.
     * <p>
     * Layout: FINETUNE_PERIODS[fineTune+8][note] fineTune : -8..+7  → index 0..15   (stored nibble in .mod: 0..15,
     * sign-extended: >7 → -16+v) note     : 0..35   (3 octaves: C-1..B-3, the range ProTracker supports)
     * <p>
     * Row  0 = finetune -8 Row  8 = finetune  0  (no detuning) Row 15 = finetune +7
     * <p>
     ...
     * <p>
     * ProTracker matches the raw period against the finetune-0 row to find the note index, then returns the value from
     * the correct finetune row.
     *
     ...
     */
    /**
     * Exact samples-per-tick as a rational number numerator/denominator. Use samplesPerTickAccurate() with a running
     * accumulator to avoid drift.
     * <p>
     * Exact value = outputRate * 2_500_000 / (BPM * 1_000_000) = outputRate * 5 / (BPM * 2)
     * <p>
     * E.g. BPM=125, rate=44100: 44100*5 / 250 = 220500/250 = 882.0 exactly — lucky! But BPM=120, rate=44100: 44100*5 /
     * 240 = 220500/240 = 918.75 — fractional. Rounding 918.75 to 919 every tick = +0.25 sample/tick error → song runs
     * 0.027% slow. Over a 3-minute song at speed=6: ~3500 rows * 6 ticks = 21000 ticks * 0.25 = 5250 extra samples =
     * ~119 ms. Fine for most songs, but some trackers are more sensitive.
     * <p>
    ...
    /**
     * Return the integer sample count for this tick and advance the sub-sample accumulator stored in PlayerState.  This
     * is the Bresenham / DDA approach: we keep the fractional remainder and add it each tick, emitting an extra sample
     * whenever it overflows 1.0 — exactly like the CIA timer counts.
     * <p>
     * tickNumerator   = outputRate * 5 tickDenominator = BPM * 2 (derived from: outputRate * 2_500_000 / (BPM *
     * 1_000_000) = outputRate*5/(BPM*2))
     */
     ...
     * <p>
     * Call this once to fill one output buffer.  It advances the song position by however many ticks fit in the
    ...
    public static void renderAudio(
        ...
        while (frameIdx < numFrames) {
            ...
            for (int s = 0; s < toMix; s++) {
                ...
                // Each side sums 2 channels, each at most 1.0, so divide by 2.0
                // to normalise to [-1, 1] before writing. Clamp catches any
                // rounding overshoot but should never actually trigger.
                outBuffer[(frameIdx + s) * 2] = clamp(left / 2.0f);
                outBuffer[(frameIdx + s) * 2 + 1] = clamp(right / 2.0f);
            }
    ...
    private static void processRow(PlayerState st, double clock, int rate) {
        ...
        for (int ch = 0; ch < 4; ch++) {
            ...
            switch (n.effect) {
                ...
                case 0xA -> {
                    if (n.effectParam != 0)
                        chn.effectMemory = n.effectParam;
                } // Axx: store param, keep last if A00
    ...
    private static void processEffectTicks(PlayerState st, double clock, int rate) {
        ...
        for (int ch = 0; ch < 4; ch++) {
            ...
            switch (n.effect) {
                ...
                case 0xA ->
                        applyVolumeSlide(chn, chn.effectMemory);                    // Axx Vol Slide (uses stored param, supports A00 continue)
            }
        }
    }
    ...
    private static void handleSetSpeedBpm(PlayerState st, int param) {
        if (param == 0)
            return;           // 0 is ignored by ProTracker
        if (param < 0x20)
