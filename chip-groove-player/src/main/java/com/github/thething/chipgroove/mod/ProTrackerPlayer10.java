 ...
 */
public class ProTrackerPlayer10 {
    ...
    static class Channel {
        ...
        // Vibrato state (4xy effect) — mirrors tremolo but offsets period instead of volume
        int    vibratoPos;     // 0..63, position in the waveform table
        int    vibratoSpeed;   // x nibble: how much vibratoPos advances per tick
        int    vibratoDepth;   // y nibble: amplitude of the pitch wobble
        int    vibratoWaveform; // 0=sine, 1=ramp down, 2=square (set via E4x, default 0)
        int    periodBeforeVibrato; // the "real" period, vibrato offsets from this temporarily

        // Delay Sample state (EDx extended effect — "Note Delay")
        boolean delayedTriggerPending; // true while waiting for the delay tick
        int     delayedTriggerTick;    // tick number (1..speed-1) at which to fire
        Sample  delayedSample;         // sample to trigger once the tick arrives
        int     delayedPeriod;         // raw period to trigger with; finetune is applied inside trigger() itself

        // Tone portamento state (3xx effect — "slide to note")
        int    portaTarget;    // period to slide toward (0 = no active slide)
        int    portaSpeed;     // amount to change period by, per tick (always positive;
        // direction is determined automatically by comparing
        // current period to portaTarget)
        ...
        void trigger(Sample s, int period, double amigaClock, int outputRate) {
            ...
            this.tremoloPos = 0;
            // Same reset rule for vibrato (E4x bit 2 controls retrigger there too).
            this.vibratoPos = 0;
            this.periodBeforeVibrato = tunedPeriod;
        }
        ...
        /**
         * 3xx Tone Portamento target-set.
         *
         * CRITICAL: this does NOT call trigger() and does NOT reset samplePos.
         * The note keeps playing from wherever it currently is; only the pitch
         * starts gliding toward the new target period. This matches real
         * ProTracker behaviour — 3xx with a note column is the one case in the
         * format where a "new note" does not restart the sample.
         *
         * Finetune IS still applied to the target, since the target period
         * comes from pattern data exactly like a normal note would.
         */
        void setPortaTarget(int rawPeriod, double amigaClock, int outputRate) {
            int tunedTarget = (sample != null && sample.fineTune != 0)
                    ? applyFineTune(rawPeriod, sample.fineTune)
                    : rawPeriod;
            this.portaTarget = tunedTarget;
            // If this channel has never played a note yet, there's nothing to
            // glide from — snap directly to the target (matches PT behaviour
            // when 3xx appears with no prior note in that channel).
            if (this.period == 0) {
                this.period = tunedTarget;
                updateIncrement(tunedTarget, amigaClock, outputRate);
            }
        }
    ...
    private static void processRow(PlayerState st, double clock, int rate) {
        ...
        for (int ch = 0; ch < 4; ch++) {
            ...
            // EDx Delay Sample: extract the sub-effect/sub-param from effectParam
            // BEFORE deciding whether to trigger normally. EDx packs into a
            // standard 0xE row as effectParam = (0xD << 4) | delayTicks.
            boolean isDelaySample = (n.effect == 0xE) && (((n.effectParam >> 4) & 0xF) == 0xD);
            int     delayTicks    = isDelaySample ? (n.effectParam & 0xF) : 0;

            // Clear any leftover pending delay from a previous row. If the
            // previous row's EDx never reached its tick (e.g. delay >= speed,
            // or speed was lowered mid-row via Fxx), it must not leak into
            // this row and fire at the wrong moment.
            chn.delayedTriggerPending = false;
            chn.delayedSample = null;

            // Trigger note (if period specified) — EXCEPT for 3xx (Tone Portamento),
            // which reuses the column's period as a *target* to glide toward
            // instead of restarting the sample. This is the one effect in the
            // format where having a note in the note column does not retrigger.
            boolean isTonePorta = (n.effect == 0x3);

            if (isDelaySample && delayTicks > 0 && delayTicks < st.speed && n.period > 0 && smp != null) {
                // Queue the trigger instead of firing now. The note keeps
                // whatever was already playing on this channel until the
                // delay tick arrives — exactly like a normal silent row would.
                chn.delayedTriggerPending = true;
                chn.delayedTriggerTick    = delayTicks;
                chn.delayedSample         = smp;
                chn.delayedPeriod         = n.period;
            } else if (n.period > 0 && smp != null && !isTonePorta) {
                chn.trigger(smp, n.period, clock, rate);
            } else if (smp != null && !isTonePorta) {
            ...
            switch (n.effect) {
                ...
                case 0x3 -> {                                                  // 3xx Tone Portamento (init on tick 0)
                    if (n.effectParam != 0) chn.portaSpeed = n.effectParam; // 0 means "keep last speed"
                    if (n.period > 0) chn.setPortaTarget(n.period, clock, rate);
                    // If no note in the note column, keep sliding toward the
                    // previously set target — common pattern: "300" continues
                    // a slide started on an earlier row with a new speed only.
                }
                ...
                case 0x4 -> {                                                  // 4xy Vibrato (init on tick 0)
                    int x = (n.effectParam >> 4) & 0xF;
                    int y =  n.effectParam       & 0xF;
                    if (x != 0) chn.vibratoSpeed = x;   // 0 means "keep last speed"
                    if (y != 0) chn.vibratoDepth = y;   // 0 means "keep last depth"
                    // Remember the period vibrato should oscillate around. If this
                    // row also had a note (handled above via trigger/setPortaTarget-
                    // style logic), chn.period is already the new note's period —
                    // capture it now before any wobble is applied.
                    chn.periodBeforeVibrato = chn.period;
                }
    ...
    private static void processEffectTicks(PlayerState st, double clock, int rate) {
        ...
        for (int ch = 0; ch < 4; ch++) {
            ...
            // EDx Delay Sample: fire the queued trigger exactly when the
            // current tick matches the requested delay. This check runs
            // before the normal effect switch so the freshly-triggered
            // channel still gets standard tick processing this same tick
            // (e.g. a delayed note can still receive vibrato on later ticks).
            if (chn.delayedTriggerPending && st.tick == chn.delayedTriggerTick) {
                chn.trigger(chn.delayedSample, chn.delayedPeriod, clock, rate);
                chn.delayedTriggerPending = false;
                chn.delayedSample = null;
            }
            ...
            switch (n.effect) {
                ...
                case 0x3 -> applyTonePorta(chn, clock, rate);                           // 3xx Tone Portamento (ticks 1+)
                case 0x4 -> applyVibrato(chn, clock, rate);                             // 4xy Vibrato (ticks 1+)
            }
        }
    }
    ...
    /**
     * 3xx Tone Portamento — glide the period toward ch.portaTarget by
     * ch.portaSpeed units per tick, never overshooting the target.
     *
     * Unlike 1xx/2xx (which slide forever in one direction with no limit),
     * tone portamento stops exactly at the target and stays there — this is
     * what makes it useful for "slide into this note" musical phrases.
     *
     * Direction is automatic: if current period > target, the pitch needs
     * to go up (lower period = higher pitch), so we subtract; otherwise add.
     */
    private static void applyTonePorta(Channel ch, double clock, int rate) {
        if (ch.portaTarget == 0 || ch.period == ch.portaTarget) return; // nothing to do
    ...
    /**
     * 4xy Vibrato — oscillates pitch (period) around the channel's base note.
     *
     * ProTracker formula:
     *   delta  = (depth * waveformValue(waveform, pos)) / 128
     *   period = periodBeforeVibrato + delta
     *
     * Note the divisor is 128 here, not 64 like tremolo — vibrato's depth
     * nibble has finer granularity since periods span a much larger numeric
     * range (113..856) than volume (0..64). This matches the original
     * ProTracker source scaling exactly.
     *
     * Like tremolo, this NEVER permanently changes ch.period's "real" value —
     * periodBeforeVibrato is the anchor (set on note trigger or by 4xy's own
     * tick-0 handler), and vibrato recomputes the offset fresh every tick.
     * This means vibrato and tone portamento (3xx) can't run on the same
     * channel simultaneously without conflict — real ProTracker has a
     * combined effect (5xy) for that case, not implemented here.
     */
    private static void applyVibrato(Channel ch, double clock, int rate) {
        int waveform = ch.vibratoWaveform;
        int raw      = waveformValue(waveform, ch.vibratoPos);   // -255..255
        int delta    = (ch.vibratoDepth * raw) / 128;
        int newPeriod = ch.periodBeforeVibrato + delta;
        ch.updateIncrement(newPeriod, clock, rate);
        // Note: ch.period itself is left untouched (still the "real" note
        // period) — only the playback increment reflects the wobble. This
        // is important so that a later effect reading ch.period (e.g. a
        // pattern break or another note trigger) sees the true note, not
        // a vibrato-displaced value.
        ch.vibratoPos += ch.vibratoSpeed;
        // vibratoPos wraps naturally via & 63 inside waveformValue()
    }
